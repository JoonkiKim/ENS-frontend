...
export default function MyPage() {
  ...
  // react-hook-form 초기화
  const { register, watch, setValue, reset, control, handleSubmit } = useForm<FormData>({
    defaultValues: {
      name: '',
      phone: '',
      emailLocal: '',
  ...
  const { data: majorsData } = useQuery<{ fetchAllMajors: Array<{ id: string; name: string; isCustom: boolean }> }>(FETCH_ALL_MAJORS, {
    fetchPolicy: 'cache-first', // 캐시 우선 사용, 없으면 네트워크 요청
  });
  const { data: industriesData } = useQuery<{ fetchAllIndustries: Array<{ id: string; name: string; isCustom: boolean }> }>(FETCH_ALL_INDUSTRIES, {
    fetchPolicy: 'cache-first', // 캐시 우선 사용, 없으면 네트워크 요청
  });
  const { data: positionCategoriesData } = useQuery<{ fetchAllPositionCategories: Array<{ id: string; name: string; isCustom: boolean }> }>(FETCH_ALL_POSITION_CATEGORIES, {
    fetchPolicy: 'cache-first', // 캐시 우선 사용, 없으면 네트워크 요청
  });
  ...
  const positionCategories = [
    ...(positionCategoriesData?.fetchAllPositionCategories?.map(pc => pc.name) || []),
    '기타'
  ];
  ...
  const { data: loginUserData, loading: loginLoading } = useQuery(FETCH_LOGIN_USER);
  ...
  useEffect(() => {
    ...
    const isLoggedIn = loginUserData?.fetchLoginUser !== undefined && !loginLoading;
    if (!isLoggedIn) {
      return;
    }
    ...
    // 이메일 완성 여부 확인
    const hasEmail = emailLocal && emailDomain && emailLocal.trim() && emailDomain.trim();
  ...
  }, [formValues.emailLocal, formValues.emailDomain, targetUserId, router, loginUserData, loginLoading, emailRequiredModal]);
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.userMajors && user.userMajors.length > 0) {
        const sortedMajors = [...user.userMajors].sort((a, b) => 
      ...
      if (user.careers && user.careers.length > 0) {
        careersData = user.careers.map((career, index) => {
          const startDate = career.startDate ? new Date(career.startDate) : null;
  ...
  const [careerDropdownStates, setCareerDropdownStates] = useState<Record<number, {
  ...
  const handleAddMajor = () => {
    const newId = majorFields.length > 0 ? Math.max(...majorFields.map((_, idx) => majors[idx]?.id || idx + 1)) + 1 : 1;
  ...
  const handleAddCareer = () => {
    const newId = careerFields.length > 0 ? Math.max(...careerFields.map((_, idx) => careers[idx]?.id || idx + 1)) + 1 : 1;
    appendCareer({
      id: newId,
      isCurrentJob: false,
      isPrivate: false,
  ...
  // 직무 카테고리 선택
  const handleSelectPosition = (careerIndex: number, positionCategory: string) => {
    ...
    if (positionCategory !== '기타') {
      ...
      if (positionCategoryId) {
        setValue(`careers.${careerIndex}.positionCategoryId`, positionCategoryId);
      }
    } else {
      // '기타'인 경우 ID 제거
  ...
  const handleSelectMonth = (careerIndex: number, month: number) => {
    ...
    setCareerDropdownStates(prev => ({
      ...
      [careerIndex]: {
        ...prev[careerIndex],
        isMonthDropdownOpen: false,
        isDatePickerOpen: currentCareer?.joinYear && month ? false : prev[careerIndex]?.isDatePickerOpen,
      },
    }));
  };
  ...
  const handleSelectLeaveMonth = (careerIndex: number, month: number) => {
    ...
    if (updatedLeaveYear && month) {
      ...
      setCareerDropdownStates(prev => ({
        ...prev,
        [careerIndex]: { ...prev[careerIndex], isLeaveMonthDropdownOpen: false },
      }));
    }
  };
  ...
  useEffect(() => {
    ...
    // 드롭다운이 열려있는지 확인
    const hasOpenDropdown = Object.values(careerDropdownStates).some(
      (state) => 
  ...
  }, [careerDropdownStates, isGenerationDropdownOpen, isStudentIdDropdownOpen, isEmailDomainDropdownOpen]);
  ...
  const transformFormDataToUpdateInput = (formData: FormData) => {
    ...
    const careers = formData.careers
      .filter(career => career.company && career.company.trim() && career.position && career.position.trim())
      .map(career => {
        // 입사년월을 ISO 문자열로 변환
        ...
        if (career.positionCategory) {
          if (career.positionCategory === '기타' && career.customPositionCategoryName) {
            // 사용자가 '직접 입력'을 선택하고 커스텀 이름을 입력한 경우
            customPositionCategoryName = career.customPositionCategoryName.trim() || null;
            ...
            positionCategoryId = positionCategoryMap[career.positionCategory] || null;
          }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.errors && result.errors.length > 0) {
        const errorMessage = result.errors[0].message || '변경사항 저장 중 오류가 발생했습니다.';
        setErrorModal({
          isOpen: true,
...
setValue('confirmPassword', '');
  ...
  if (loading) {
    return (
      <S.Container>
        <S.Hero>
          <S.HeroTitle>{targetUserId ? '학회원 정보 수정' : '마이페이지 · 내 정보 수정'}</S.HeroTitle>
        </S.Hero>
        ...
        <S.Section>
          ...
          <S.ProfileSection>
            ...
            <S.ProfileInfo>
              ...
              <S.StatusBadges>
                <S.StatusBadge active={watch('noCoffeeChat')} onClick={handleToggleCoffeeChat}>
                  커피챗은 어려워요
                </S.StatusBadge>
                <S.StatusBadge active={watch('abroad')} onClick={handleToggleOverseas}>
                  해외 거주 중
                </S.StatusBadge>
              </S.StatusBadges>
            </S.ProfileInfo>
                    ...
                    <S.MajorInputWrapper key={field.id} data-major-search>
                      <div style={{ position: 'relative', width: '413px' }}>
                        <S.Input 
                          ...
                          value={majorValue}
                          onChange={(e) => handleMajorSearch(index, field.id, e.target.value)}
                          onFocus={() => handleMajorInputFocus(index, field.id, majorValue)}
                          data-major-search
                        />
                        {searchState.isDropdownOpen && searchState.filteredMajors.length > 0 && (
                          ...
                          >
                            {searchState.filteredMajors.map((major) => (
                              ...
                              >
                                {highlightMajorText(major.name, searchState.searchQuery)}
                              </S.IndustryDropdownItem>
                            ))}
                          </S.IndustryDropdown>
                        )}
            ...
            <S.FormInputWrapper>
              <S.EmailField>
                ...
                <S.EmailDomainSelectField 
                  ...
                  data-dropdown-field
                >
                  {isEmailDomainCustom || (emailDomain && emailDomain !== 'gmail.com' && emailDomain !== 'naver.com') ? (
                    <S.EmailInput 
                      type="text" 
                  ...
                  <S.DropdownIcon style={{ position: 'absolute', right: '5px' }}>
                    <svg viewBox="0 0 15 11" fill="none">
                      <path d="M1 1L7.5 9L14 1" stroke="#999999" strokeWidth="2" />
                    </svg>
                  </S.DropdownIcon>
              ...
              <S.CareerItem key={field.id}>
                <S.CareerCard>
                  <S.CareerRow>
                    ...
                    <S.CareerField 
                      isClickable={true}
                      onClick={() => handleTogglePositionDropdown(careerIndex)}
                      data-dropdown-field
                    >
                      ...
                      {career?.positionCategory === '기타' ? (
                        <S.CareerInput 
                          ...
                          onClick={(e) => e.stopPropagation()}
                          {...register(`careers.${careerIndex}.customPositionCategoryName`)}
                          style={{ flex: 1 }}
                        />
                        ...
                        <S.IndustryDropdown data-dropdown-container>
                          {positionCategories.map((positionCategory) => (
                            <S.IndustryDropdownItem
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleSelectPosition(careerIndex, positionCategory);
                              }}
                            >
                              {positionCategory === '기타' ? '직접 입력' : positionCategory}
                            </S.IndustryDropdownItem>
                          ))}
                        </S.IndustryDropdown>
                      )}
                  ...
                  <S.CareerRow>
                    <S.CareerField 
                      isClickable={true}
                      onClick={() => handleToggleIndustryDropdown(careerIndex)}
                      data-dropdown-field
                    >
                      ...
                      {career?.industry === '기타' ? (
                        <S.CareerInput 
                          ...
                          onClick={(e) => e.stopPropagation()}
                          {...register(`careers.${careerIndex}.customIndustryName`)}
                          style={{ flex: 1 }}
                        />
                      ) : career?.industry ? (
                        <S.CareerValue>
                          ...
                          <S.DateSelectorWrapper>
                            <S.DateSelector data-year-selector>
                              <S.DateInput
                                ...
                                isDropdownOpen={!!dropdownState.isYearDropdownOpen}
                                onClick={(e) => {
                                  e.stopPropagation();
                            ...
                            <S.DateSelector data-month-selector>
                              <S.DateInput
                                ...
                                isDropdownOpen={!!dropdownState.isMonthDropdownOpen}
                                onClick={(e) => {
                                  e.stopPropagation();
                    ...
                    <S.CareerField 
                      ...
                      onClick={() => !career?.isCurrentJob && handleToggleLeaveDatePicker(careerIndex)}
                      data-dropdown-field
                    >
                      {career?.isCurrentJob ? (
                              ...
                              <S.DateSelectorWrapper>
                                <S.DateSelector data-year-selector>
                                  <S.DateInput
                                    ...
                                    isDropdownOpen={!!dropdownState.isLeaveYearDropdownOpen}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleToggleLeaveYearDropdown(careerIndex);
                                    }}
                                  />
                                  <S.DropdownIcon style={{ position: 'absolute', right: '18px', pointerEvents: 'none' }}>
                                    <svg viewBox="0 0 15 11" fill="none">
                                    ...
                                    <S.YearDropdown data-dropdown-container>
                                      {years.map((year) => (
                                        <S.YearDropdownItem
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            handleSelectLeaveYear(careerIndex, year);
                                          }}
                                        >
                                          {year}년
                                        </S.YearDropdownItem>
                                ...
                                <S.DateSelector data-month-selector>
                                  <S.DateInput
                                    ...
                                    isDropdownOpen={!!dropdownState.isLeaveMonthDropdownOpen}
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleToggleLeaveMonthDropdown(careerIndex);
                                    }}
                                  />
                                  <S.DropdownIcon style={{ position: 'absolute', right: '18px', pointerEvents: 'none' }}>
                                    <svg viewBox="0 0 15 11" fill="none">
                                    ...
                                    <S.MonthDropdown data-dropdown-container>
                                      {months.map((month) => (
                                        <S.MonthDropdownItem
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            handleSelectLeaveMonth(careerIndex, month);
                                          }}
                                        >
                                          {month}월
                                        </S.MonthDropdownItem>
                ...
                <S.CareerCheckboxes>
                  <S.CheckboxLabel onClick={() => handleTogglePrivate(careerIndex)}>
                  ...
                  >
                    <S.CheckboxBox disabled={false} checked={career?.isCurrentJob || false} />
                    <span>재직 중</span>
                  </S.CheckboxLabel>
                </S.CareerCheckboxes>
              </S.CareerItem>
